...
it('includes the cross-module reference and history columns', function () {
    $columns = [
        'memberships' => [
            'tenant_id', 'member_id', 'plan_id', 'branch_id', 'previous_membership_id',
            'plan_name_snapshot', 'plan_access_rules_snapshot',
        ],
        'invoices' => [
            'tenant_id', 'member_id', 'membership_id', 'subtotal_cents', 'grand_total_cents',
            'amount_paid_cents', 'balance_due_cents', 'idempotency_key',
        ],
        'payments' => [
            'tenant_id', 'invoice_id', 'method', 'amount_cents', 'idempotency_key',
        ],
        'attendance_records' => [
